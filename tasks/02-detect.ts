...
import { retry } from "@/lib/concurrency";
...
type CliLogLevel = "info" | "success" | "warn" | "error" | "dim";

function log(msg: string, level: CliLogLevel = "info"): void {
  switch (level) {
    case "success":
      consola.success(msg);
      break;
    ...
    case "error":
      ...
      break;
    case "dim":
      consola.log("\x1b[90m%s\x1b[0m", msg);
      break;
    default:
      consola.info(msg);
  }
...
async function main(): Promise<void> {
  ...
  const rows = await Promise.all(
    repos.map((repo, idx) =>
      limiter(async () => {
        const row = await detectRepo(repo, repoIndex.get(repo) ?? [], sources, opts, hintsMap, variantHintsMap);
        const now = Date.now();
